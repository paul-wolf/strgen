...
The generator will randomly choose characters from the set of lower
case letters, digits and the underscore. The number of characters
generated will be exactly one in this case. For more, use a
quantifier:

    [a-zA-Z0-9_]{8}

As a shortcut for commonly used character sets, a character set code
may be used. The following will render in exactly the same way:
...
characters or a string of digits that is four in length.
...
they occur.  Use of the `|` or `&` operators always binds the operands
...
time. The maximum number of times it will try is by default
...
Call the dump() method on the class instance to get useful
information:
